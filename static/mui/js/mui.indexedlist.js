...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		findElements: function() {
			var self = this;
			self.el = self.el || {};
			self.el.search = mui(classSelector('indexed-list-search'))[0];
			self.el.searchInput = mui(classSelector('indexed-list-search-input'))[0];
			self.el.searchClear = mui(classSelector('indexed-list-search') + ' ' + classSelector('icon-clear'))[0];
		...
		caleLayout: function() {
			...
			var withoutSearchHeight = (self.box.offsetHeight - 0) + 'px';
